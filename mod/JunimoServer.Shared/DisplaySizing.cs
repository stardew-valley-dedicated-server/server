...
public static class DisplaySizing
{
    ...
    public static void ApplyFromEnv(IMonitor monitor)
    {
        try
        {
            var w = int.TryParse(Environment.GetEnvironmentVariable("DISPLAY_WIDTH"), out var dw)
                ? dw
                : DefaultWidth;
            var h = int.TryParse(Environment.GetEnvironmentVariable("DISPLAY_HEIGHT"), out var dh)
                ? dh
                : DefaultHeight;
