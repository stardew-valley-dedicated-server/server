        ...
        )
        {
            chatCommandsService.RegisterCommand(
                ...
                (args, msg) =>
                {
                    ...
                    if (
                        args.Length == 1
                        && args[0].Equals("separate", StringComparison.OrdinalIgnoreCase)
                    )
                    {
                        wantSeparate = true;
                    }
                        ...
                        args.Length == 1
                        && args[0].Equals("shared", StringComparison.OrdinalIgnoreCase)
                    )
                    {
                        wantSeparate = false;
                    }
                    ...
                    if (wantSeparate == isSeparate)
                    {
                        if (flipScheduled)
                        {
                            ...
                            helper
                                .GetMultiplayer()
                                .globalChatInfoMessage(
                                    isSeparate ? "MergeWalletsCancel" : "SeparateWalletsCancel",
                                    actorName
                                );
                        }
                        else
                        {
                            helper.SendPrivateMessage(
                                msg.SourceFarmer,
                                wantSeparate
                                    ? "Wallets are already separate."
                                    : "Wallets are already shared."
                            );
                        }
                        return;
                    }
