    ...
    )
    {
        ...
        chatCommandsService.RegisterCommand(
            ...
            (args, msg) =>
            {
                ...
                var inviteCode =
                    InviteCodes.Joinable ?? $"not yet available ({InviteCodes.UnavailableReason})";
                ...
                helper.SendPrivateMessage(
                    msg.SourceFarmer,
                    $"In-Game: {CultureInfo.InvariantCulture.TextInfo.ToTitleCase(season)} {Game1.dayOfMonth}, Year {Game1.year} - {FormatGameTime(Game1.timeOfDay)}"
                );
                ...
                helper.SendPrivateMessage(
                    msg.SourceFarmer,
                    $"Rendering: {(renderingFps > 0 ? $"On ({renderingFps} fps)" : "Off")}"
                );
