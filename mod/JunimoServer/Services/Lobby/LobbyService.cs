...
public class LobbyService : ModService
{
    ...
    /// </summary>
    private readonly ConcurrentDictionary<string, (Color day, Color night)> _originalCabinLighting =
        new();
    ...
    private bool TryRestoreInventoryFromBackup(EditingSessionBackup backup)
    {
        ...
        if (!string.IsNullOrEmpty(backup.PreviousLocation))
        {
            Game1.server.sendMessage(
                ...
                Multiplayer.passout,
                Game1.player,
                new object[] { backup.PreviousLocation, backup.PreviousX, backup.PreviousY, false }
            );
        }
