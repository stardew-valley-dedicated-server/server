...
internal sealed class RecordingOrchestrator
{
    ...
    private record ContainerUsage(string ContainerId, string Label, long MarkTimestamp, long MarkRunMs);
    ...
    /// </summary>
    public Task MarkContainerUsedAsync(string containerId, string label, CancellationToken ct = default)
    {
        if (!RecordingPolicy.IsEnabled) return Task.CompletedTask;
        _usages.Add(new ContainerUsage(containerId, label, Stopwatch.GetTimestamp(), RunMetadata.GetRunMs()));
    ...
    public async Task FinalizeAsync(
        string testClass, string testMethod, string displayName, bool testFailed,
        CancellationToken ct = default)
    {
        if (!RecordingPolicy.IsEnabled || _usages.Count == 0) return;
        ...
        var deduped = _usages
            .GroupBy(u => (u.ContainerId, LabelPrefix: u.Label.StartsWith("server") ? "server" : "client"))
        ...
        if (RecordingPolicy.Mode == TestRecordingMode.Failure && !testFailed)
        {
            ...
            {
                InfrastructureEventLog.Emit("recording_per_test_clip_skipped", new
                {
                ...
                SetupEventBus.EmitRecordingSkipped(testClass, testClass, displayName,
                    usage.Label, RecordingSkipReason.RetentionPassed);
            }
            return;
        }
        ...
        foreach (var usage in deduped)
        {
            ...
            {
                endEpochs[usage.ContainerId] =
                    recorder.ContainerEpochAtHostTicks(endAbsoluteTs);
            }
        }
        ...
        var clientIndex = 0;
        var extractionJobs = new List<(ContainerUsage usage, ContainerRecorder recorder,
            string fileName, string destPath, double startEpoch, double durationSec,
            double timelineOffset, double wallDuration, long endRunMs, double markEpoch)>();
        ...
        foreach (var usage in deduped)
        {
            ...
            if (recorder == null)
            {
                ...
                {
                    container = usage.Label,
                    reason = "recorder_missing"
                });
                SetupEventBus.EmitRecordingSkipped(testClass, testClass, displayName,
                    usage.Label, RecordingSkipReason.RecorderMissing);
                continue;
            }
            if (!recorder.IsRecording)
            {
                ...
                {
                    container = usage.Label,
                    reason = "recorder_never_started"
                });
                SetupEventBus.EmitRecordingSkipped(testClass, testClass, displayName,
                    usage.Label, RecordingSkipReason.RecorderNeverStarted);
                continue;
            }

            if (usage.Label.StartsWith("server") && !RecordingPolicy.RecordServer) continue;
            if (usage.Label.StartsWith("client") && !RecordingPolicy.RecordClient) continue;
            ...
            if (!endEpochs.TryGetValue(usage.ContainerId, out var endEpoch))
            {
                ...
                {
                    container = usage.Label,
                    reason = "end_time_missing"
                });
                SetupEventBus.EmitRecordingSkipped(testClass, testClass, displayName,
                    usage.Label, RecordingSkipReason.EndTimeMissing);
                continue;
            }
            ...
            if (durationSec <= 0)
            {
                ...
                SetupEventBus.EmitRecordingSkipped(testClass, testClass, displayName,
                    usage.Label, RecordingSkipReason.ZeroDuration);
                continue;
            }

            // Wall-clock duration: absolute time from mark to end snapshot.
            // Used by the UI for clip width on the timeline.
            var wallDuration = Stopwatch.GetElapsedTime(usage.MarkTimestamp, endAbsoluteTs).TotalSeconds;
            ...
            var timelineOffset = Stopwatch.GetElapsedTime(minMarkTs, usage.MarkTimestamp).TotalSeconds;

            extractionJobs.Add((usage, recorder, fileName, destPath, startEpoch, durationSec,
                timelineOffset, wallDuration, endRunMs, markEpoch));
        }
        ...
        var extractionOutputs = await Task.WhenAll(extractionJobs.Select(async job =>
        {
            ...
            try
            {
                ...
                var result = await job.recorder.ExtractClipFromLiveAsync(
                    job.startEpoch, job.durationSec, job.destPath, ct);
                sw.Stop();
                return (job, source, encodedWith, result, elapsedMs: sw.ElapsedMilliseconds, exception: (Exception?)null);
            }
            catch (Exception ex)
            {
                return (job, source, encodedWith,
                    result: new ContainerRecorder.ExtractionResult(null, null),
                    elapsedMs: 0L, exception: ex);
            }
        }));
        ...
        var tasks = extractionOutputs.Select(async eo =>
        {
            var job = eo.job;
            ...
            try
            {
                ...
                if (result.HostPath != null)
                {
                    ...
                    var effectiveTimelineOffset = (result.ActualFirstFramePts is double pts
                                                    && pts > 1e9
                                                    && alignmentBaseEpoch is double basePts)
                        ? pts - basePts
                        : job.timelineOffset;

                    SetupEventBus.EmitRecording(testClass, testClass, displayName, path, source,
                        effectiveTimelineOffset, job.wallDuration);

                    // Field semantics live in the InfrastructureEventLog event-catalog comment.
                    InfrastructureEventLog.Emit("recording_per_test_clip", new
                    {
                ...
                {
                    InfrastructureEventLog.Emit("recording_per_test_clip_failed", new
                    {
                        container = job.usage.Label,
                        fileName = job.fileName,
                        source,
                    ...
                    SetupEventBus.EmitRecordingSkipped(testClass, testClass, displayName,
                        source, RecordingSkipReason.ExtractionFailed);

                    SetupEventBus.EmitTestAnnotation(displayName, AnnotationLevel.Error,
