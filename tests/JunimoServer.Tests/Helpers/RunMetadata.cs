...
public static class RunMetadata
{
    ...
    public static void BeginRun()
    {
        ...
        if (!string.IsNullOrEmpty(externalRunDir))
        {
            ...
            runId = Path.GetFileName(
                externalRunDir.TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)
            );
        }
        else
        {
    ...
    /// </summary>
    internal static void WriteRunMetadata(
    ...
    )
    {
        ...
        var metadata = new RunMetadataJson
        {
            ...
            ServerConfigs = demands
                .Select(d => new ServerConfigInfo
                {
                    Key = d.Key,
        ...
        try
        {
            ArtifactPrettyJson.Write(
                Path.Combine(runDir, RunArtifactNames.RunMetadataJson),
                metadata
            );
        }
        catch (Exception ex)
        ...
        try
        {
            SetupEventBus.EmitRunMetadata(metadata);
        }
        catch (Exception ex)
        {
            TestLog.Test($"[RunMetadata] Failed to emit run_metadata event: {ex.Message}");
        }
    }
    ...
    private static Dictionary<string, string?> GetEnvInfo()
    {
        var keys = new[]
        {
            "SDVD_IMAGE_TAG",
            "SDVD_STOP_ON_FAIL",
            "SDVD_MAX_CONCURRENT_STARTS",
            "SDVD_MAX_CONCURRENT_EXTRACTIONS",
        };
