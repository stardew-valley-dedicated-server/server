...
internal static class TestLog
{
    internal static void Server(string message) =>
        Console.Error.WriteLine(
            $"[Server] {DateTime.UtcNow:HH:mm:ss.fff} {PathDisplay.ScrubMessage(message)}"
        );

    internal static void Client(string message) =>
        Console.Error.WriteLine(
            $"[Client] {DateTime.UtcNow:HH:mm:ss.fff} {PathDisplay.ScrubMessage(message)}"
        );

    internal static void Test(string message) =>
        Console.Error.WriteLine(
            $"[Test]   {DateTime.UtcNow:HH:mm:ss.fff} {PathDisplay.ScrubMessage(message)}"
        );
