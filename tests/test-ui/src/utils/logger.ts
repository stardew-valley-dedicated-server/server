...
export function createLogger(scope: string): Logger {
  const prefix = `[${scope}]`
  return {
    log:   (...args) => { if (isEnabled(scope)) console.log(prefix, ...args) },
    warn:  (...args) => { if (isEnabled(scope)) console.warn(prefix, ...args) },
    error: (...args) => { if (isEnabled(scope)) console.error(prefix, ...args) },
  }
}
