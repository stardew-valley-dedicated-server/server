import type { OutputEntry, AnnotationLevel, AnnotationSource } from '../types/state'
...
export type OutputSegment =
  | { type: 'lines'; items: AnnotationEntry[] }
  | { type: 'images'; items: ScreenshotEntry[] }
...
export function annotationSourceIcon(source: AnnotationSource): string {
  switch (source) {
    case 'broker':    return 'lucide:settings'
    case 'recording': return 'lucide:video'
    case 'mod':       return 'lucide:puzzle'
    case 'setup':     return 'lucide:hammer'
    case 'body':
    default:          return 'lucide:chevron-right'
  }
}
...
export function annotationLevelClass(level: AnnotationLevel): string {
  switch (level) {
    ...
    case 'detail':  return 'text-base-content/55'
    case 'trace':   return 'text-base-content/35'
    case 'section': return 'text-info font-semibold'
    case 'info':
    default:        return 'text-base-content/75'
  }
}
...
export function annotationSourceClass(source: AnnotationSource): string {
  switch (source) {
    case 'broker':    return 'text-info/70'
    case 'recording': return 'text-warning/70'
    case 'mod':       return 'text-success/70'
    case 'setup':     return 'text-base-content/40'
    case 'body':
