...
export function tpsLevel(value: number, targetTps: number | null): TpsLevel {
    ...
    if (value >= target * TPS_GOOD_RATIO) {
        return "good";
    }
    if (value >= target * TPS_WARN_RATIO) {
        return "warn";
    }
    return "error";
}
...
export function queueTextClass(stats: MetricStats | undefined): string {
    if (!stats || stats.pendingActions == null) {
        return "text-base-content/40";
    }
    if (stats.pendingActions === 0) {
        return "text-base-content/50";
    }
    if (stats.pendingActions <= 5) {
        return "text-warning";
    }
    return "text-error";
}
...
export function waitTextClass(stats: MetricStats | undefined): string {
    if (!stats || stats.gameThreadWaitMs == null) {
        return "text-base-content/40";
    }
    if (stats.gameThreadWaitMs < 50) {
        return "text-base-content/50";
    }
    if (stats.gameThreadWaitMs < 200) {
        return "text-warning";
    }
    return "text-error";
}
