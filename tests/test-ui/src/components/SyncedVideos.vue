...
import { useFilmstripCache, defaultFrameCount, THUMB_W, THUMB_H, type Thumbnail } from '../composables/useFilmstripCache'
import { DEFAULT_TARGET_TPS, TPS_PEAK_HEADROOM, TPS_TARGET_HEADROOM, METRIC_COLORS, tpsLevel } from '../utils/metrics'
...
const { timelinePos: sharedTimelinePos, playing: sharedPlaying, breakpoints, hitBreakpointLine, hoveredBreakpointSec } = useVideoTimeline()
watch(timelinePos, (v) => { sharedTimelinePos.value = v })
watch(playing, (v) => { sharedPlaying.value = v })
...
const totalDuration = computed(() => {
  ...
  let max = lc && (lc.testMs + lc.artifactsMs + lc.cleanupMs) > 0
...
const rulerTicks = computed(() => {
  ...
  const mag = Math.pow(10, Math.floor(Math.log10(minSec)))
...
function tpsGraph(index: number): TpsGraphData | null {
  ...
  for (const p of pts) {
    ...
    const x = toX(p.offsetSec), y = toY(p.tps)
...
function snappedTpsAt(index: number, timelineSec: number): { tps: number; target: number; offsetSec: number } | null {
  ...
  for (const p of pts) {
    ...
    if (dist < nearestDist) { nearestDist = dist; nearest = { offsetSec: p.offsetSec, tps: p.tps } }
  }
...
function findNextBreakpoint(prevPos: number, nextPos: number): { lineNum: number; sec: number } | null {
  ...
  for (const [lineNum, sec] of breakpoints.value) {
    ...
    if (sec >= prevPos && sec <= nextPos) {
      if (best === null || sec < best.sec || (sec === best.sec && lineNum < best.lineNum)) best = { lineNum, sec }
    }
  }
  return best
}
...
function togglePlay() {
  if (playing.value) {
    ...
    for (const el of videoRefs.value) { if (el && !el.paused) el.pause() }
  } else {
    stopped.value = false
...
function setSpeed(speed: number) {
  ...
  for (const el of videoRefs.value) { if (el) el.playbackRate = speed }
}
...
function onTrackMouseMove(e: MouseEvent, trackIndex: number) {
  ...
  for (const bp of breakpointMarkers.value) {
    if (snapBreakpointSec(bp.sec) === snap) { matched = bp.sec; break }
  }
...
function mountPreviewCanvas(canvas: HTMLCanvasElement | null, trackIndex: number) {
  if (!canvas) { previewCanvasCtx = null; previewCanvasTrack = -1; return }
...
function downloadAll() { props.videos.forEach((_, i) => downloadVideo(i)) }
...
onMounted(() => {
  ...
  if (scrollContainerRef.value) {
    ...
    resizeObserver = new ResizeObserver(entries => {
      ...
      resizeTimer = window.setTimeout(() => { resizing.value = false }, 100)
    })
...
watch(() => props.videos, () => {
  ...
  nextTick(() => { crossfading.value = false })
  crossfadeTimer = setTimeout(() => { prevThumbnails.value = new Map() }, 350)
