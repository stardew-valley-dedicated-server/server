...
import { useTestUI, useFilterTrigger } from '../composables/useTestUI'
import { useTestSort } from '../composables/useTestSort'
import { useTestFilter, allStatuses, type TestStatus } from '../composables/useTestFilter'
...
const { activeFilters, toggleFilter, visibleStatuses, isFiltering, testPassesFilter, setExclusiveFilter } = useTestFilter(statusCounts, searchQuery)
...
const timelineTests = computed(() => {
  ...
  const executed = all.filter(t => t.executionOrder != null)
    .sort((a, b) => a.executionOrder! - b.executionOrder!)
  const notExecuted = all.filter(t => t.executionOrder == null)
    .sort((a, b) => {
      const da = a.discoveryOrder ?? Infinity
      const db = b.discoveryOrder ?? Infinity
...
const sharedSetupPhases = computed(() =>
  store.state.setupPhases.filter(p => p.collectionName == null)
)
