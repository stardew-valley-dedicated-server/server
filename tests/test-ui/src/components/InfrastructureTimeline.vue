...
import { registerChartPlugins, GRID_COLOR, TICK_COLOR, X_TICK_COLOR } from '../utils/chart'
...
const chartData = computed<ChartData<'scatter', TimelinePoint[]>>(() => {
  ...
  const buckets: Record<CategoryKey, TimelinePoint[]> = {
    created: [], leased: [], returned: [], poisoned: [], disposed: [],
  }
...
watch(() => allInstances.value.map(i => i.instanceId).join('|'), () => { renderKey.value++ })
</script>
