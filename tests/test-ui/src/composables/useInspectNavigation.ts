...
export function useInspectNavigation(params: {
  ...
  const inspectId = computed(() => inspectStack.value.length > 0 ? inspectStack.value[inspectStack.value.length - 1] : null)
  ...
  const hasNextInspect = computed(() => inspectIndex.value >= 0 && inspectIndex.value < allOrderedInstances.value.length - 1)
  ...
  const inspectInstance = computed(() => {
    ...
    return params.instances.value.find(i => i.instanceId === inspectId.value)
  ...
  return {
    inspectStack, inspectId, canGoBack,
    inspectIndex, hasPrevInspect, hasNextInspect,
    allOrderedInstances,
    openInspect, pushInspect, closeInspect, goBackInspect,
    prevInspect, nextInspect,
    inspectInstance, inspectPeers,
  }
}
