...
import { formatTimeLabel, colorFor } from '../utils/chart'
...
export function useChartPipeline(params: {
  ...
  function flushChartData() {
    ...
    if (newSet.size !== instancesWithStats.value.size ||
    ...
    const allDatasets: Record<MetricKey, any[]> = { cpu: [], mem: [], tps: [], fps: [], queue: [], wait: [], gc: [], netRx: [], netTx: [] }
    ...
    for (const inst of instances) {
      ...
      const instStart = histEpochs.length > 0 ? histEpochs[0] : Infinity
      const instEnd = histEpochs.length > 0 ? histEpochs[histEpochs.length - 1] : -Infinity
  ...
  watch(() => params.statsVersion.value, () => {
    scheduleFlush()
  })
