...
export const allStatuses = ['running', 'queued', 'passed', 'failed', 'canceled', 'skipped', 'notDispatched', 'aborted', 'pending'] as const
export type TestStatus = typeof allStatuses[number]

export function useTestFilter(
  ...
  const visibleStatuses = computed(() =>
    allStatuses.filter(s => statusCounts[s] > 0)
  )
