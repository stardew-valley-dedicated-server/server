...
export interface TopologyGroup { server: InstanceSnapshot; clients: InstanceSnapshot[] }
...
export function useTopologyLines(params: {
  ...
  const connectionLines = ref<{ groupIdx: number; x1: number; y1: number; x2: number; y2: number; active: boolean }[]>([])

  function updateConnectionLines() {
    if (params.layout.value !== 'topology' || !params.topologyContainerRef.value || params.viewportWidth.value <= 600) {
  ...
  watch([params.topologyGroups, params.unconnectedClients, params.layout, params.viewportWidth], () => {
