...
export function useTestStore(): TestStore {
    ...
    const {
        elapsedTimerRef,
        elapsedMs,
        startElapsedTimer: _startTimer,
        stopElapsedTimer: _stopTimer,
    } = useElapsedTimer();
    ...
    function applyEvent(event: TestEvent) {
        switch (event.event) {
            ...
            case "test_failed": {
                const test = findOrCreateTest(event.testCollection, event.testClass, event.displayName);
                const isCanceled =
                    event.exceptionType === "System.OperationCanceledException" ||
            ...
            case "setup_phase_started": {
                ...
                const existingPhase = state.setupPhases.find(
                    (p) => makePhaseKey(p.category, p.phase, p.collectionName) === key,
                );
            ...
            case "setup_phase_completed": {
                ...
                const phase = state.setupPhases.find(
                    (p) => makePhaseKey(p.category, p.phase, p.collectionName) === key,
                );
            ...
            case "instance_leased": {
                ...
                if (inst) {
                    ...
                    inst.history.push({
                        ...
                        reason: null,
                        serverInstanceId: event.serverInstanceId ?? null,
                        clientInstanceId: null,
                    });
                }
            ...
            case "instance_client_attached": {
                ...
                if (inst) {
                    inst.history.push({
                        ...
                        reason: null,
                        serverInstanceId: null,
                        clientInstanceId: event.clientInstanceId ?? null,
                    });
                }
                break;
            }
            ...
            case "instance_stats": {
                const statsEntry = {
                    ...
                    totalMemoryMb: event.totalMemoryMb,
                    fps: event.fps ?? null,
                    tps: event.tps ?? null,
    ...
    return {
        ...
        set selectedError(v) {
            selectedError.value = v;
        },
        lastSelectionWasAuto,
        get isConnected() {
            return ws?.isConnected.value ?? false;
        },
        get connectionError() {
            return ws?.connectionError.value ?? null;
        },
        get isReportMode() {
