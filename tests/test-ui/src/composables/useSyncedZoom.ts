...
export function useSyncedZoom(opts?: { enabled?: Ref<boolean> }) {
  ...
  function trackChartRef(el: any) {
    if (el == null) { pruneDead(); return }
    if (!chartEls.includes(el)) chartEls.push(el)
  }

  const ctrlHeld = ref(false)
  function onKeyDown(e: KeyboardEvent) { if (e.key === 'Control') ctrlHeld.value = true }
  function onKeyUp(e: KeyboardEvent) { if (e.key === 'Control') ctrlHeld.value = false }
  ...
  function checkZoomed() {
    for (const c of chartEls) {
      const chart = c?.chart
      if (!chart) continue
      if (chart.isZoomedOrPanned?.()) { isZoomed.value = true; return }
    }
  ...
  function syncAllAxes(sourceChart: any) {
    if (syncing || !enabled.value) { checkZoomed(); return }
    pruneDead()
