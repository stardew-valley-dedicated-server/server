...
function syncHandle(h: InternalHandle) {
  ...
  if (clipHost) {
    ...
    const clipPath = clipTop > 0 || clipBottom > 0
...
function onScroll() { scheduleSyncAll() }
function onResize() { scheduleSyncAll() }
...
export function setClipHost(host: HTMLElement | null) {
  clipHost = host
  if (clipHostRO) { clipHostRO.disconnect(); clipHostRO = null }
...
export function createIframeSlot(): IframeHandle {
  ...
  function track(el: HTMLElement) {
    ...
    if (h.placeholderRO) { h.placeholderRO.disconnect(); h.placeholderRO = null }
