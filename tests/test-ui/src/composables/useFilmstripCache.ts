...
export interface Thumbnail { url: string }
...
async function generate(job: ThumbJob) {
  ...
  if (!loaded) { log.warn(`failed to load video ${job.path.slice(-40)}`); cleanupVideo(vid); return }

  const duration = vid.duration
  if (!duration || !isFinite(duration) || duration <= 0) { cleanupVideo(vid); return }
  ...
  if (remaining.length === 0) { cleanupVideo(vid); return }

  log.log(`${resumeFrom > 0 ? 'resuming' : 'generating'} ${remaining.length}/${seekTimes.length} thumbs for ${job.path.slice(-40)}`, { src: job.src.slice(0, 80) })
  ...
  cleanupVideo(vid)
  log.log(`done ${job.path.slice(-40)} (${thumbs.length} thumbs${resumeFrom > 0 ? `, resumed from ${resumeFrom}` : ''})`)
}
