import { ref } from 'vue'
import type { TestSnapshot, ClassSnapshot } from '../types/state'

export type SortMode = 'none' | 'name-asc' | 'name-desc' | 'duration-asc' | 'duration-desc' | 'started-asc' | 'started-desc' | 'finished-asc' | 'finished-desc'
...
export function useTestSort() {
  ...
  function sortTests(tests: TestSnapshot[]): TestSnapshot[] {
    ...
    if (mode === 'duration-asc') return sorted.sort((a, b) => (a.durationMs ?? Infinity) - (b.durationMs ?? Infinity))
    if (mode === 'duration-desc') return sorted.sort((a, b) => (b.durationMs ?? -Infinity) - (a.durationMs ?? -Infinity))
    if (mode === 'started-asc') return sorted.sort((a, b) => (a.runningStartTime ?? '\uffff').localeCompare(b.runningStartTime ?? '\uffff'))
    if (mode === 'started-desc') return sorted.sort((a, b) => (b.runningStartTime ?? '').localeCompare(a.runningStartTime ?? ''))
    if (mode === 'finished-asc') return sorted.sort((a, b) => (testFinishTime(a) ?? Infinity) - (testFinishTime(b) ?? Infinity))
    if (mode === 'finished-desc') return sorted.sort((a, b) => (testFinishTime(b) ?? -Infinity) - (testFinishTime(a) ?? -Infinity))
    return sorted
  }
    ...
    if (mode === 'name-asc' || mode === 'name-desc') {
      ...
      const aggDuration = (cls: ClassSnapshot) => {
        ...
        for (const t of cls.tests) {
          if (t.durationMs == null) return mode === 'duration-asc' ? Infinity : -Infinity
          total += t.durationMs
        }
        return cls.tests.length > 0 ? total : (mode === 'duration-asc' ? Infinity : -Infinity)
      }
      ...
      const latestFinish = (cls: ClassSnapshot) => {
        let max = mode === 'finished-asc' ? Infinity : -Infinity
        for (const t of cls.tests) {
