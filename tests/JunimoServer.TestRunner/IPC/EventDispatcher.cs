...
{
    private static readonly Dictionary<string, Action<JsonElement, ITestRenderer>> Routes =
        new()
        {
            [EventNames.SetupStarted] = (e, r) => r.OnSetupPhaseStarted(Deserialize<SetupPhaseStartedEvent>(e)),
            [EventNames.SetupCompleted] = (e, r) => r.OnSetupPhaseCompleted(Deserialize<SetupPhaseCompletedEvent>(e)),
            ...
            [EventNames.Screenshot] = (e, r) => r.OnScreenshotCaptured(Deserialize<ScreenshotCapturedEvent>(e)),
            [EventNames.Recording] = (e, r) => r.OnRecordingCaptured(Deserialize<RecordingCapturedEvent>(e)),
            [EventNames.RecordingSkipped] = (e, r) => r.OnRecordingSkipped(Deserialize<RecordingSkippedEvent>(e)),
            [EventNames.VncUrl] = (e, r) =>
            {
                var evt = Deserialize<VncUrlEvent>(e);
                if (!string.IsNullOrEmpty(evt.Url)) r.OnVncUrl(evt);
            },
            [EventNames.InstanceCreated] = (e, r) => r.OnInstanceCreated(Deserialize<InstanceCreatedEvent>(e)),
            [EventNames.InstanceLeased] = (e, r) => r.OnInstanceLeased(Deserialize<InstanceLeasedEvent>(e)),
            [EventNames.InstanceClientAttached] = (e, r) => r.OnInstanceClientAttached(Deserialize<InstanceClientAttachedEvent>(e)),
            [EventNames.InstanceReturned] = (e, r) => r.OnInstanceReturned(Deserialize<InstanceReturnedEvent>(e)),
            [EventNames.InstanceDisposed] = (e, r) => r.OnInstanceDisposed(Deserialize<InstanceDisposedEvent>(e)),
            [EventNames.InstanceRecording] = (e, r) => r.OnInstanceRecording(Deserialize<InstanceRecordingEvent>(e)),
            [EventNames.InstancePoisoned] = (e, r) => r.OnInstancePoisoned(Deserialize<InstancePoisonedEvent>(e)),
            [EventNames.InstanceConnected] = (e, r) => r.OnInstanceConnected(Deserialize<InstanceConnectedEvent>(e)),
            [EventNames.InstanceDisconnected] = (e, r) => r.OnInstanceDisconnected(Deserialize<InstanceDisconnectedEvent>(e)),
            [EventNames.InstanceStats] = (e, r) => r.OnInstanceStats(Deserialize<InstanceStatsEvent>(e)),
            [EventNames.TestOutput] = (e, r) => r.OnTestOutput(Deserialize<TestOutputEvent>(e)),
            [EventNames.TestAnnotation] = (e, r) => r.OnTestAnnotation(Deserialize<TestAnnotationEvent>(e)),
            [EventNames.TestEnrichment] = (e, r) => r.OnTestEnrichment(Deserialize<TestEnrichmentEvent>(e)),
            [EventNames.RunMetadata] = (e, r) =>
            {
                if (!e.TryGetProperty("data", out var dataEl)) return;
                var runDir = e.TryGetProperty("runDir", out var rd) && rd.ValueKind == JsonValueKind.String
                    ? rd.GetString() ?? "" : "";
                r.OnRunMetadata(new RunMetadataEvent(runDir, dataEl.Clone()));
            },
            [EventNames.FlakyTests] = (e, r) =>
            {
                if (!e.TryGetProperty("tests", out var tests)) return;
                r.OnFlakyTests(new FlakyTestsEvent(tests.Clone()));
            },

            // ── xUnit lifecycle events (snake_case wire keys) ──

            [EventNames.DiscoveryComplete] = (e, r) => r.OnDiscoveryComplete(Deserialize<DiscoveryCompleteEvent>(e)),
    ...
    public static void Dispatch(string json, ITestRenderer renderer)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (ex is not JsonException) throw;
        }
    }

    private static T Deserialize<T>(JsonElement el) where T : IRendererEvent
    {
