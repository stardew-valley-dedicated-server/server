...
Players connect with unmodded vanilla clients (CLAUDE.md), so a patch reaches only code executing in the **server process**. Patching a method that vanilla resolves client-side for the behavior you're changing — `Game1.warpFarmer`'s local warp resolution, menu logic, totem targets, `getMailboxPosition` during interaction — changes nothing for farmhands; only the host's own calls see it. The per-player-farms plan initially routed farmhand warps via a `Game1.warpFarmer` prefix — dead code for every farmhand, caught only on re-review. Before proposing a patch, classify where the target method executes for the affected actor: server-simulated logic (NPC movement, overnight processing, night events, save/load) is patchable; client-side resolution must instead ride the server→client control surface ([`vanilla-client-control-primitives.md`](vanilla-client-control-primitives.md)).
