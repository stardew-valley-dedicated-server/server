...
What is **not** a driver: the empty server's clock. `SetTime`/`SetClockSpeed` with nobody connected never reach the 2:00 AM pass-out — the world is paused, and even unpaused the lone-host `shouldTimePass` tail advanced only intermittently ([`host-automation.md`](host-automation.md) invariant 11).
