...
**Every test server generates a new game.** Each test gets a fresh per-runId saves volume (`sdvd-test-saves-{runId}`, `ServerContainer.cs`) → `HasLoadableSave()` is always false → `CreateNewGameFromConfig()` (`GameManagerService`). Only the `!loadedGame`-only delta (NewDay/flush/GenerateBundles) is avoidable via a pre-baked save; the map-build floor runs on both the load and create paths.
