...
- **polybar + resize-handler boot in parallel** with `app` and don't gate startup (their `disabled` scripts return false = enabled, for the runtime rendering-toggle button).
