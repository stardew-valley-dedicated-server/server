...
| `git-workflow.md` | Project-specific git rules (no `git add .`, no Co-Authored-By trailers, strict up-to-date merge + `!approve` self-approval, chained PRs rebased `--onto`, PR descriptions, every bot review thread ends resolved with an in-thread reply, build-before-`--continue` rebasing, worktrees at `../worktrees/` via the WorktreeCreate hook) |
