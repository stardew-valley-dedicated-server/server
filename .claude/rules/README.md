...
| `dotnet-appdata-folder-needs-existing-xdg-dir.md` | `docker/**`, `mod/**/*.cs`, `tests/test-client/**` | `GetFolderPath(ApplicationData)` returns "" unless the XDG config dir exists — writes land relative to the service cwd; root hides it, a non-root uid crashes SMAPI — create the dir in the root init hook |
