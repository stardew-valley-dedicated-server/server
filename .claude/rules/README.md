...
| `git-workflow.md` | Project-specific git rules (no `git add .`, no Co-Authored-By trailers, strict up-to-date merge + `!approve` self-approval, chained PRs rebased `--onto`, PR descriptions, every bot review thread ends resolved (in-thread reply on rejection or when a fix does not auto-resolve), build-before-`--continue` rebasing, worktrees at `../worktrees/` via the WorktreeCreate hook) |
