...
| `autopause-is-the-sole-ispaused-writer.md` | `mod/JunimoServer/**` | `HandleAutoPause` is the only writer of `IsPaused` — mask the flag around a serialization or let presence unpause; a one-tick unpause banks clock time an unauthenticated client can farm |
