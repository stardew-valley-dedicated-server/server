...
| `deployment-config-is-env-not-settings-file.md` | `.github/workflows/**`, `docker-compose.yml`, `Env.cs`, `Services/Settings/**` | A per-deployment knob is an env var from a GitHub Environment variable, never a shipped `server-settings.json` (the mod writes world-coupled state back into that file); operator knobs are feature-first and unprefixed, `SDVD_` is test-harness/kill-switch only |
