...
| `git-workflow.md` | Git rules: explicit staging, SSH only, `!approve` + squash (or per-line rebase) merge, chained PRs, rebase builds, changelog-facing commit/PR format, bot threads, worktrees |
...
|---|---|---|
| `apiservice-entry-is-conditional.md` | `mod/JunimoServer/**` | `ApiService.Entry` returns before subscribing anything when `API_ENABLED=false` — shared infrastructure (dispatcher, scheduler) owns its own `UpdateTicked` subscription, never an `ApiService` handler |
...
| `autopause-is-the-sole-ispaused-writer.md` | `mod/JunimoServer/**` | `HandleAutoPause` is the sole lifecycle writer of `IsPaused` — others may only mask around a serialization or unpause a now-present player (incl. synchronously at auth); a stray one-tick unpause banks clock time an unauthenticated client can farm |
...
| `cabin-system.md` | `CabinManager/`, `GameLoader/`, `GameCreator/` | Eleven cabin-allocation invariants: unconditional patches with per-message gating, EnsureAtLeastXCabins, SlotSelectionGate, coordinates, master-only build, live-vs-persisted Farmer split, claim sweep, protected-slot assignment, staged migration |
...
| `deployment-config-is-env-not-settings-file.md` | `.github/workflows/**`, `docker-compose.yml`, `Env.cs`, `Services/Settings/**` | A per-deployment knob is an env var from a GitHub Environment variable, never a shipped `server-settings.json` (the mod writes world-coupled state back into that file); operator knobs are feature-first and unprefixed, `SDVD_` is test-harness/kill-switch/compose-plumbing only |
...
| `dotnet-appdata-folder-needs-existing-xdg-dir.md` | `docker/**`, `mod/**/*.cs`, `tests/test-client/**`, `tools/xnb-unpacker/**` | `GetFolderPath(ApplicationData)` returns "" unless the XDG config dir exists — writes land relative to the service cwd; root hides it, a non-root uid crashes SMAPI — create the dir in the root init hook |
...
| `host-automation.md` | `AlwaysOnServer/`, `HostAutomation/`, `Lobby/`, `CabinManager/` | Decompiled-first; `hasDedicatedHost = false`, `netReady` formula, festival repro caveat, draw-coupled FarmEvent completion, host farmhouse internal-only, empty-server pause held off only by host activity (event, warp, pass-out), grace sleep past 1:00 AM |
...
| `privilege-drop-sets-home-after-drop.md` | `docker/**`, `tools/**/Dockerfile*`, `tools/**/entrypoint.sh` | `gosu`/`su-exec` reset HOME to the passwd home (`/` for unknown uids) — set `env HOME=` after the drop, pre-create what the app writes, boot once with an arbitrary uid; Windows bind mounts always look root-owned, check a named volume |
...
| `runner-ui-pipeline-plumbing.md` | `SetupEventBus.cs`, `ContainerStatsCollector.cs`, `TestRunner/**`, test-ui types/store | Adding a field to a runner→UI event needs end-to-end plumbing — every hop is hand-written, and the pipe silently drops JSONL lines over 4096 chars |
...
| `smapi-global-data-lives-on-saves-volume.md` | `mod/JunimoServer/**` | Global data is `.smapi/mod-data/<mod-uid>/` on the saves volume, keyed by mod id — survives `/newgame`, `/reload`, and folder deletion, dies with the volume; pick the store by the lifetime the data must have |
...
| `test-day-transition-needs-connected-driver.md` | `tests/**/*.cs` | Day transitions have two drivers — a connected sleeper, or the empty-server grace sleep past 1:00 AM (`SetTime(2550)`) — never the empty server's clock |
