...
| `recorder-anchor-first-frame.md` | `tests/**/Helpers/ContainerRecorder.cs`, `Helpers/Recording*.cs` | Load-bearing recorder invariants: `-tune zerolatency`, MPEG-TS segments (not MKV), wall-clock-honest PTS, segments.csv as anchor source (not ffprobe — TS PTS wraps), phase-lock preamble, two-pass extraction, sequence-number filenames (not strftime). Each is the fix for a specific observed failure mode |
