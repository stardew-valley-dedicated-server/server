...
The pipe also hard-caps line size: `SetupPipeServer.MaxLineBytes` (4096 bytes) silently drops any oversized JSONL line — including test annotations from `TestBase.Log` — leaving only a `setup_ipc_oversized_line` event in `infrastructure.parent.jsonl`. Bound big payloads (exec output, captured container logs) before emitting them; an 11.7 KB annotation vanished this way while its test stayed green, which would have cost the triage data on a failure.
