...
**Why:** Verifying the overlay burn-in, repeated full-width `format=gray` rawvideo scans reported phantom black pixels at the canvas edge (e.g. "rightmost black col x=599" on a 600px frame) that flatly contradicted direct `crop=1:1` RGB reads showing that same pixel was pure white `255 255 255`. The full-frame scan's indexing assumptions are easy to get silently wrong (frame dimensions, pixel format, row alignment, luma threshold — a lavfi `color=white` source doesn't always read as luma 255 under the chosen threshold), and its results repeatedly contradicted direct crop reads. The per-column `crop=1:H` + rgb24 reads were immediately reliable and repeatable.
