...
- **Volumes**: create via Docker.DotNet (`DockerOps.CreateVolumeAsync`) with labels; Testcontainers 4.3.0 has no `VolumeBuilder.WithLabel`.
...
**Every Testcontainers builder must call `.WithDockerEndpoint(host.EndpointConfig)`** (`Helpers/DockerEndpointConfig.cs`, owned per host on `DockerHost`). Testcontainers 4.3.0 builds `DockerClientConfiguration` with no timeout overrides, so Docker.DotNet's defaults apply — including `namedPipeConnectTimeout: 100ms`, which fails individual API calls under parallel-startup pressure on Windows (Docker Desktop's named-pipe accept queue saturates).

**Symptom of the missing endpoint config**: `TimeoutException("The operation has timed out.")` from `_serverContainer.StartAsync` with empty `containers/server-N/container.log` and elapsed << configured `StartupTimeout`. **Do not fix this with retries** — fix it by setting `namedPipeConnectTimeout` to ~5s via `host.EndpointConfig`. Linux/CI uses Unix sockets and is unaffected, but the call is safe everywhere.
