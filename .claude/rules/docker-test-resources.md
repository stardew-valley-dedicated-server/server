...
**Docker stats callbacks run concurrently — use `Interlocked` on shared per-instance state.** `GetContainerStatsAsync` delivers samples through `Progress<T>`, which posts each callback to the thread pool when no synchronization context exists (probed: 6 callbacks in flight at once), so callbacks overlap and can run out of order — especially in the burst after a stall. Guard mutable `InstanceEntry` fields with `Interlocked`; for a last-seen timestamp use a monotonic compare-exchange loop (advance only when the new value is newer), not a bare `Exchange` — a read-then-write reported the same stream gap twice, and an `Exchange` can move the value backward.
...
**Why:** The endpoint timeout fix was reached by the user pushing back on a proposed retry loop ("hint: retries", "no bandaids"). The retry was masking a 100ms timeout default that surfaces only under Windows + parallel startup pressure — exactly the conditions a CI run never hits but local dev always does. See [`dont-paper-over-root-cause.md`](universal/dont-paper-over-root-cause.md) for the meta-rule.
