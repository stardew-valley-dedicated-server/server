...
**Why:** ~30 min lost during the streaming refactor. All three call sites (`ServerContainer`, `GameClientContainer`, `SharedSteamAuth`) launch the streaming task **before** `container.StartAsync` per the `SuppressFlow` pattern ([`asynclocal-pitfalls.md`](asynclocal-pitfalls.md)). Testcontainers' `IContainer.Id` getter unblocks during *create* (when the daemon assigns an ID), well before *start* — so the streaming task's first `GetContainerLogsAsync` call typically races into the small window between create and run. Initial reader treated the immediate EOF as natural end-of-life and exited; smoke test reported `1 passed` while every `container.log` was empty (zero bytes, zero `forwardedVia` events in `infrastructure.jsonl`). Verified via standalone repro: pre-start `Follow=true` reliably returns EOF; post-start works as documented.
