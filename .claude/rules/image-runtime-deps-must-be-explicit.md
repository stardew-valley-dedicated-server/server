...
**How to apply:** After any package removal, run the image far enough that the app process actually starts (not just container init). When adding a runtime dependency, install it explicitly with a comment naming the consumer, so a future removal of an unrelated package can't orphan it. Sibling of the artifact-inspection gate in `universal/runtime-post-conditions-are-gates.md` (build green ≠ artifact correct); this is the runtime-dependency case.
