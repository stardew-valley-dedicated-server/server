...
**How to apply:** In a root init hook or entrypoint, create the *app user's* config dir before dropping privileges and chown it to `USER_ID:GROUP_ID` — `/config/xdg/config` on the jlesage images, `$APP_HOME/.config` where HOME is set explicitly. Never derive it from the hook's own `$HOME`, which is root's. This applies to every image that runs the game or a SMAPI-hosted process (server, test client, unpacker). When a game-side write ends up at a path relative to the service directory, suspect an empty `GetFolderPath` result before suspecting the caller.
