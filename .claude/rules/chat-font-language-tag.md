...
## The render-suppressed server still LOADS localized fonts (measure path)

The headless server suppresses rendering (`SERVER_FPS=0`, `Game.Draw` patched out in `ServerOptimizerOverrides.Draw_Prefix`), but it still loads SpriteFonts via a non-render **measure** path: `Multiplayer.receiveChatMessage` → `ChatBox.receiveChatMessage` → `parseMessageForEmoji` → `new ChatSnippet(text, language)`, whose ctor calls `ChatBox.messageFont(language).MeasureString(...)` (a parse-time width measurement) → `Game1.content.Load<SpriteFont>("Fonts\\SmallFont", <message-language>)`. So a non-English client's chat makes the *server* load `SmallFont.<lang>` on the game thread and throw `ContentLoadException` if that file was stripped from the download — server-side, not just client-side (#260).

**"Rendering patched out" does NOT imply "no font loads."** MeasureString (wrapping/layout), parse, and anything touching a SpriteFont's metrics run on the game thread independent of `Game.Draw` — separate *draw* paths (suppressed) from *measure/parse* paths (still run) when reasoning about what a headless server loads.
