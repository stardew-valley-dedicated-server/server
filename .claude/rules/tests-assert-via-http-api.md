...
Mod `Diagnostics.ModEventLog.Emit(...)` events (`cabin_*`, `auth_*`, …) are transported via `Console.Out` `SDVD_EVENT ` lines → `SimpleContainerLogStreamer` → `infrastructure.jsonl` on disk. They are **diagnostics only**. No test asserts on a mod-emitted event, and there is **no "wait for mod event X" API** — the event stream is not a test assertion surface.
