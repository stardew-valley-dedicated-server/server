...
Status: open. This is a conflation + naming cleanup. No timeout **value** changes anywhere.
...
The name reads as "delete timeout" but governs *appear* and *disappear* polls too. The fix routes each site to a semantically-correct home that already exists — no new constants, no value changes.
...
read-only `/players` snapshot (<50ms), so 35s is dead headroom.

This path does **not** poison on timeout — it returns `false` and logs a warning
(`TestLifecycle.cs:607-612`), so the impact is hygiene/consistency. The disappear-poll
default change below fixes it for free, but the call site should still pass the budget
explicitly to match its siblings and read clearly.

## Finding 2 — three stale comments cite a "15s" game-thread timeout / "~16s" 503

The real `RunOnGameThreadAsync` timeout is **5s** (`mod/JunimoServer/Services/Api/ApiService.cs:1746`); a 503 means a ~5s game-thread block, and the measured delete cost is ~0.1s. Current wording, verbatim:
...
Repoint each conflated default to its correct existing home, fix the one disconnect-wait call site, and correct the comments. Grouped by file.
...
Leave **`WaitForFarmhandDeletedByNameAsync`** (`:2581`) on `FarmerDeleteTimeout` — a genuine delete-retry, the constant's now-sole meaning.

### B. `tests/JunimoServer.Tests/Infrastructure/Fixture/TestLifecycle.cs` (~line 603)

Pass the disappear-poll budget explicitly (matches siblings; independent of A1/A2):
...
- **`ServerApiTests.cs:38`** (`GetPlayers_WhenNoPlayersConnected`, waits for `/players`==0
  while a *prior* test's cleanup drains): pass `TestTimings.ServerReadyBetweenTests` (30s),
  the established "wait for prior-test teardown to settle" budget, instead of the delete
  constant. If a flake appears, fall back to an explicit 35s.
- **`FarmhandManagementTests.cs:109`** (polls `/farmhands` for own just-deleted name to
  disappear, after `WaitForFarmhandDeletedByNameAsync` at `:87` already confirmed the
  delete): pass `TestTimings.FarmerRemovalBudget` (2s) to match the disappear-poll
  semantics, OR keep `FarmerDeleteTimeout` if 2s proves tight. Decide from the verification
  run.

### D. `tests/JunimoServer.Tests/Helpers/TestTimings.cs` — comment corrections

- **`FarmerDeleteTimeout` (`:87-95`)**: state it is the **delete-retry** budget only; real
  game-thread timeout is 5s and measured delete cost ~0.1s, so 35s is ample retry headroom.
  Drop "(15s)" / "~16s".
- **`CabinAssignmentTimeout` (`:201-206`)**: state the real driver — customization/
...
1. `dotnet build tests/JunimoServer.Tests/JunimoServer.Tests.csproj` — compile.
2. `grep -n "15s\|~16s\|timeout (15" tests/JunimoServer.Tests/Helpers/TestTimings.cs` → **zero** hits.
3. `grep -n "FarmerDeleteTimeout" tests/` → only `WaitForFarmhandDeletedByNameAsync` (`:2581`), the `TestLifecycle` delete loop (`:738`), and the constant itself. No disappear/appear poll references it.
4. Run the affected classes — `make test FILTER=FarmhandManagementTests` and
   ...
   `poll_completed` for `Polling_ServerApi_WaitForPlayersRemovedById` shows `timeoutMs: 2000`,
   and `Polling_ServerApi_WaitForFarmhandByName` shows `timeoutMs: 35000`.
5. If `ServerApiTests` or `FarmhandManagementTests:109` flake on the tightened budget, widen
   that one site and note it.
...
- **No new constants** — every semantic already has a correct home.
- **Not renaming `CabinAssignmentTimeout`** despite it now backing non-cabin farmhand waits.
  A rename touches 30+ call sites; do it as its own follow-up if desired.
