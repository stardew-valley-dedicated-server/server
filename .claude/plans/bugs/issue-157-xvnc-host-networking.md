...
**Area:** docker
**Related:** [`issue-157-steam-proxy-support.md`](issue-157-steam-proxy-support.md)
**Observed:** reporter's log on #157, only under the non-default `network_mode: host`; not reproduced here
**Next step:** comment on #157 (base-image-owned timeout, avoid host networking) coordinated with the companion plan's Step 6, then close as environment-specific or relabel base-image/environment

Companion plan [`issue-157-steam-proxy-support.md`](issue-157-steam-proxy-support.md) verifies the reporter's
...
Not a repo bug — base-image/environment, and only under the non-default `network_mode: host`.
...
- The only xvnc file this repo owns is
  ...
  `jlesage/baseimage-gui:debian-11-v4.11.3` (the `FROM` in `docker/Dockerfile`) and its
...
1. Run (or fold into) [`issue-157-steam-proxy-support.md`](issue-157-steam-proxy-support.md) so the reply can say "you
