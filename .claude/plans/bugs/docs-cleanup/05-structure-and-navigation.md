...
**Observed:** docs audit 2026-06-13 (structure + dead-link sweep)
**Next step:** start after 01–04 merge; run the relative-link sweep as the gate
...
- **Copies:** `players/commands.md` (general + !login), `admins/operations/commands.md`
...
- **Fix:** one canonical home per command class — player chat commands → `players/commands.md`;
  ...
  (the admin page already does this correctly for lobby customization).
...
- Both cover `steam-auth setup`, invite-code retrieval via attach-cli/`info`, and the Co-op connect
  steps (`installation.md` vs `first-setup.md`). A sidebar reader gets the same
...
- **N1. `config.ts` players sidebar is the only section without an Overview entry** —
  add `{ text: "Overview", link: "/players/" }` (admins, features, developers,
  community all have one).
- **N2. No `srcExclude`** — `docs/README.md` is built into the public site as `/README.html` and
  indexed by local search (config.ts has no srcExclude; search.provider is `local`). Add
...
- `docs/index.md` cards: the "Cross-Platform" card means host OS ("Linux or Windows") — omits
  macOS (prerequisites.md supports it via Docker Desktop) and collides with
