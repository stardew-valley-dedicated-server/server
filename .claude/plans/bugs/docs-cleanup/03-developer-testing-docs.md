...
**Next step:** open a PR for H1–H5; flag the CLAUDE.md step-count and code-comment touchpoints in the PR body
...
- **Problem:** `E2E_VNC_PORT`, `E2E_CLIENT_VNC_PORT`, `E2E_API_PORT`, `E2E_CLIENT_API_PORT`,
  ...
  `VNC_PORT`/`API_PORT`/`GAME_PORT`/`QUERY_PORT`, `docker-compose.yml`); compose's
...
- **Problem:** `SDVD_DOCKER_HOSTS` is required — the runner throws at startup when unset
  (`HostPool.cs`); `STEAM_ACCOUNTS` is needed for Steam tests. A new contributor running
...
- **Problem:** doc says `health.slow_response` (>3s); code emits `health.slow_tick` when
  `lastTickMs > 5000` within the healthy (<30s) branch (`ManagedServer.cs`;
  catalog `InfrastructureEventLog.cs`). Grepping the documented name finds nothing.
...
- **Problem:** the supported mechanism is `GAME_PATH` in `.env`, read by
  `Directory.Build.props` and consumed as `$(GamePath)`
  (`JunimoServer.csproj` has no GamePath property); CLAUDE.md explicitly prohibits hardcoding
  it. Following the doc dirties a tracked file.
- **Fix:** replace steps 2-3 with: set `GAME_PATH` in `.env` (documented at `.env.example`,
...
- **Problems (three confirmed):**
  1. `config.ts` statically imports `../assets/openapi.json`, which is **gitignored**
     (`docs/.gitignore`) and generated from the server image — `npm run dev` on a fresh clone
     fails at config load. Never mentioned; neither are `make docs` (Makefile) or
     `docs/scripts/fetch-openapi.sh`.
  2. The toolchain is **Bun** (bun.lock, no package-lock.json; Makefile and
     `build-docs.yml` both use bun) — the README prescribes npm throughout.
...
- **M1. Recording pipeline outdated.** Extraction is now two-pass (concat stream-copy → seek with
  re-encode libx264/NVENC, `ContainerRecorder.cs`), bounded per-host by
...
- **M2. Output tree misplaces `flakiness.jsonl`.** Cross-run file at `TestResults/flakiness.jsonl`
  (root), not per-run (`FlakinessTracker.cs`; `Makefile`). Same fix in `test-harness.md`
  per-run list.
- **M3. CI Usage omits the nightly trigger.** Fourth entry point: cron `0 8 * * *` full suite on
  master for the README badge (`e2e-tests.yml`); "all manual" is wrong (the never-a-merge-gate
...
- **M6. Multi-Host section duplicates `remote-host-setup.md` and has drifted.** e2e copy lists only
  ...
  concurrentStarts, concurrentExtractions (`HostPool.cs`; `.env.test.example`).
...
- **M7. Per-test observability stale.** `SaveLogsToFileAsync` doesn't exist (doc-only grep hit);
  ...
  `containers/{server-N|client-N|steam-auth-*}/container.log` (`TestArtifacts.cs`;
  `make test-container-log`).
- **M8. Component catalog predates multi-host.** ServerContainer no longer manages steam-auth
  (per-host `SharedSteamAuth`, "not per-server" — `ServerContainer.cs`); ClientPool is per-host
...
- **M9. Per-host resources table wrong twice.** (1) runId suffixes are per-resource random 8-char
  GUIDs (`SharedSteamAuth.cs`; `TestNetworkManager.cs`) — nothing run-wide ties them;
  (2) the game-data volume is created **without labels** (`GameDataDistributor.cs`) so the
  `sdvd.test` sweep never touches it — that's why it survives as a cross-run cache
  (`EmergencyCleanup.cs`). Fix both cells.
- **M10. STEAM_ACCOUNTS slicing claims contradict the slicer.** Once `remaining < 2`, every later
  ...
  (`SteamAccountSlicer.cs`). NOTE: the slicer's own comments  carry the same
...
- **M11. Sizing rule cites a method that doesn't exist.** No
  ...
  correct. (Stale comment at `TestResourceBroker.cs` — same companion pass.)
...
- **M13. Never mentions `make test-diagnose`** (dumps last `failure_context` — purpose-built for
  step-1/2 triage, `Makefile`) **or `make test-metadata`**. Add both.
...
- `test-harness.md`: `test_enrichment` producer site is `TestFailureReporter.cs` (invoked from
