...
| D5 | OpenAPI spec carries zero `parameters`/`requestBody`/`securitySchemes`; generator never emits them (`OpenApiGenerator.cs:50-115`) | Document params manually in `introduction.md` | Extend the generator (the generator is reflection-invoked with fixed positional args at Docker build time — see the `<remarks>` on `OpenApiGenerator.Generate`) |
