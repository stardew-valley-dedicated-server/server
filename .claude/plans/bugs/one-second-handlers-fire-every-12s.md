...
- **Per-handler wall-clock gates in mod code** — self-gate each handler with an inline
  ...
  subscription. Would work (it matches `MapService.cs:71`, `AlwaysOnFestivals.cs:260`, `ApiService.cs:1226`)
...
1. **`src/SMAPI/Framework/SCore.cs`** — replace the hardcoded `60` with the configured divisor read
   ...
   `OneSecondUpdateTicking` (`SCore.cs:1134`) and `OneSecondUpdateTicked` (`:1149`). Replace with:
   ```csharp
   ...
   **Verified against 4.5.2 source:** `SCore` holds config as `private readonly SConfig Settings`
   (`SCore.cs:313`), so `this.Settings.OneSecondTickInterval` is correct.)
...
`OneSecondTickInterval` must equal `SERVER_TPS`. Since `SERVER_TPS` is a mod-read env var (`Env.cs:35`)
...
```
(**Verified against source:** exact string match confirmed at `Game1.cs:2742`; `SGameLogger.Error`'s
...
the XACT log):
- `docker/Dockerfile:46` (server, build-time)
- `docker/Dockerfile.test-client:53` (test-client, build-time)
- `docker/modern/Dockerfile:152` (musl/Alpine image, build-time)
- `docker/rootfs/startapp.sh:166` (server runtime fallback)
- `docker/rootfs-test-client/startapp.sh:58` (test-client runtime fallback)
- `docker/modern/rootfs/opt/bin/start-game.sh:165` (musl runtime fallback)
...
- **Rule update:** `.claude/rules/one-second-update-ticked-fires-per-game-tick.md` and its citation in
  ...
  preserving the `host-automation.md:29` cross-link. Do NOT delete the rule — vanilla SMAPI still behaves
