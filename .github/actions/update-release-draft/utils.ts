...
export const GIT_MAIL = 'julian@vallee-design.de';
export const GIT_NAME = 'Julian Vallée';
export const GIT_REPO = 'stardew-valley-dedicated-server/server';
export const GIT_HEAD = 'stardew-valley-dedicated-server';
...
export async function updatePR(prNumber: string, releaseNotes: string) {
    return $fetch(`https://api.github.com/repos/${GIT_REPO}/pulls/${prNumber}`, {
        method: 'PATCH',
        body: {
            body: releaseNotes,
        },
...
export async function tryBumpAndCreateBranch(manifest: any, newVersion: string) {
    ...
    if (await branchExists(`v${newVersion}`)) {
        ...
        core.info(`Branch 'v${newVersion}' created and pushed.`);
    }
}
