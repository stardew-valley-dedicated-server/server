...
test("resetReRunCheckbox unchecks while preserving markers, and is a no-op when unchecked", () => {
    const rows = [
        { n: 1, result: "passed", resultEmoji: "🟢", sha: "abc", filter: "Foo", when: "t", runUrl: "r", reportUrl: "" },
    ];
    const idle = helper.renderBody({
        status: "passed",
        statusEmoji: "🟢",
...
test("history is capped and stays well under the comment limit across many runs", () => {
    ...
    let body = helper.renderBody({
        status: "queued",
        statusEmoji: "🟡",
        headline: "x",
        filter: "SomeLongTestClassName",
        history: [],
        requested: false,
    });
    for (let i = 1; i <= 200; i++) {
        ...
        hist.push({
            ...
            sha: "abc1234",
            filter: "SomeLongTestClassName",
            when: "2026-06-05 10:00Z",
            runUrl: "https://github.com/o/r/actions/runs/1234567890",
            reportUrl: "https://reports.example.com/e2e/branch-name/1234567890_abcdef/index.html",
        });
...
test("deriveResult: failures win over passes; the skipped clause appears only when non-zero", () => {
    const withSkips = helper.deriveResult({
        summary: { passed: 8, failed: 3, skipped: 2 },
        jobStatus: "failure",
        sha: "sha",
    });
    ...
    const noSkips = helper.deriveResult({
        summary: { passed: 8, failed: 3, skipped: 0 },
        jobStatus: "failure",
        sha: "sha",
    });
...
test("deriveResult: a reportUrl is appended to the headline; absent leaves it untouched", () => {
    const withUrl = helper.deriveResult({
...
test("findSticky matches only comments carrying the marker", async () => {
    assert.equal(
        await helper.findSticky({
            github: fakeGithub({ id: 5, body: "no marker" }),
            owner: "o",
            repo: "r",
            issue_number: 1,
        }),
...
test("isMaintainer: 404 (non-collaborator) is a deny, not an error", async () => {
    assert.equal(
        await helper.isMaintainer({ github: fakeGithubPerm({ status: 404 }), owner: "o", repo: "r", username: "u" }),
        false,
    );
});
