...
# Runs as root after the base image's 10-nginx.sh and 10-vnc-password.sh. Keeps the VNC web
# interface (nginx, port 5800) and the raw VNC port (Xvnc, port 5900) unreachable from outside the
# container unless a VNC password is configured or ALLOW_INSECURE_SETUP=true opts out.
#
...
# refuses to start rather than boot with the VNC web UI open.
...
if [ "${ALLOW_INSECURE_SETUP:-}" = "true" ]; then
    echo "VNC_PASSWORD is not set, but the VNC web UI and VNC port stay reachable because ALLOW_INSECURE_SETUP=true."
    exit 0
fi
...
# Rebind every listen address to loopback. nginx emits three forms: IPv4:port, [IPv6]:port, and bare port.
