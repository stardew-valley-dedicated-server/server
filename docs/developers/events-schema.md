...
- `testId` — the originating test's display name, carried on the `X-Test-Id` request header (sanitized to printable ASCII, which is the identity for every current name) and bound as `ModRequestContext.TestId` for the request duration. This is the **server/sidecar-side** counterpart to `test.*`: `test.*` is populated only on `test-harness` events, but a forwarded mod event can't hold the nested `test` object, so `testId` gives those events per-test attribution. Attached at every tracing level (unlike `requestId`), so even reads are attributable. Null when no HTTP request is in scope: production, background work, and game-network activity such as a join or chat message from a test's client, which reaches the server outside any request even though a test caused it. Join it to `test.displayName`.
