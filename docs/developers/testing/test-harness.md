...
The E2E test assembly fails fast with a clear error if invoked outside the runner (e.g. via plain `dotnet test`); use `make test FILTER=…` instead. Unit tests (`tests/SteamService.Tests/` and `tests/JunimoServer.UnitTests/`, which links the game-free mod sources it covers) need neither Docker nor Steam and run with `make test-unit`.
